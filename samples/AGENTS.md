...
| `nuget.config` restates `*` → nuget.org beside its own mappings | Once it maps anything, the root `NuGet.config`'s mapping no longer reaches the sample's other packages, and restore fails `NU1100` on each | restoring the sample |
